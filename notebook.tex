...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
/home/mp/anaconda3/lib/python3.6/site-packages/sklearn/cross\_validation.py:41: DeprecationWarning: This module was deprecated in version 0.18 in favor of the model\_selection module into which all the refactored classes and functions are moved. Also note that the interface of the new CV iterators are different from that of this module. This module will be removed in 0.20.
  "This module will be removed in 0.20.", DeprecationWarning)
/home/mp/anaconda3/lib/python3.6/site-packages/sklearn/learning\_curve.py:22: DeprecationWarning: This module was deprecated in version 0.18 in favor of the model\_selection module into which all the functions are moved. This module will be removed in 0.20
  DeprecationWarning)
...
{\color{incolor}In [{\color{incolor}7}]:} \PY{c+c1}{\PYZsh{} TODO: Minimum price of the data}
        \PY{c+c1}{\PYZsh{}minimum\PYZus{}price = prices.min(0)}
...
variable is predictable. * R2 score of 0.40 means that 40 percent of the
variance in Y is predictable from X.

    \textbf{Answer:} Yes, the model predicts 92.3\% of the variation in the
True Value. R\^{}2 ranges from 0 to 1, so the model explains 92.3\% of
the variance and leaves only 7.7\% of the variance unexplained.
...
{\color{incolor}In [{\color{incolor}10}]:} \PY{c+c1}{\PYZsh{} TODO: Import \PYZsq{}train\PYZus{}test\PYZus{}split\PYZsq{}}
         ...
         \PY{n}{X\PYZus{}train}\PY{p}{,} \PY{n}{X\PYZus{}test}\PY{p}{,} \PY{n}{y\PYZus{}train}\PY{p}{,} \PY{n}{y\PYZus{}test} \PY{o}{=} \PY{n}{train\PYZus{}test\PYZus{}split}\PY{p}{(}\PY{n}{features}\PY{p}{,} \PY{n}{prices}\PY{p}{,} \PY{n}{test\PYZus{}size}\PY{o}{=}\PY{l+m+mf}{0.20}\PY{p}{,} \PY{n}{random\PYZus{}state}\PY{o}{=}\PY{l+m+mi}{2}\PY{p}{)}
...
curves are converging.
    ...
    \textbf{Answer: } The maximum depth of the top right graph is 3. Scores
improve as training points are added. Both scores and variance tend to
stabilize after about 300 training points, which suggests that adding
training points would not necessarily benefit the model.
...
it shows a low score and suffers from high bias and is underfitting.
When the maximum depth is 10 the model is less biased than training with
a max depth of 1, but variance is high. Since the model performs will
with the training data but has a high variance with the validation data,
...
\textbf{Answer: } Parameter 'max\_depth' is 4 for the optimal model.
This confirms my original expectation in Question 6 above.
...
for each client's home.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}11}]:} \PY{c+c1}{\PYZsh{} Produce a matrix for client data}
...
Predicted selling price for Client 1's home: \$415,800.00 Predicted
selling price for Client 2's home: \$236,478.26 Predicted selling price
for Client 3's home: \$888,720.00

The hint has a reasonable explanation for clients 2 and 3. Client 1's
size, poverty, and schools lie between the other two clients, and the
price is also in the middle. The estimate for Client 1 is also similar
to the sample mean and median. All estimates are within the sample
minimum and maximum range. If estimates were outside this range, it
would through the estimates into question.
...
trained on.}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}12}]:} \PY{n}{vs}\PY{o}{.}\PY{n}{PredictTrials}\PY{p}{(}\PY{n}{features}\PY{p}{,} \PY{n}{prices}\PY{p}{,} \PY{n}{fit\PYZus{}model}\PY{p}{,} \PY{n}{client\PYZus{}data}\PY{p}{)}
\end{Verbatim}
